...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		var foldersToCheck:Array<String> = [Asset2File.getPath + Paths.getPreloadPath('scripts/')];
		...
		if (OpenFlAssets.exists(file)) {
			dialogueJson = DialogueBoxPsych.parseDialogue(Asset2File.getPath + file);
		}

		var file:String = Paths.txt(songName + '/' + songName + 'Dialogue'); //Checks for vanilla/Senpai dialogue
		if (OpenFlAssets.exists(file)) {
			dialogue = CoolUtil.coolTextFile(Asset2File.getPath + file);
		}
		...
		#if LUA_ALLOWED
		var filesPushed:Array<String> = [];
		var foldersToCheck:Array<String> = [Asset2File.getPath + Paths.getPreloadPath('data/' + Paths.formatToSongPath(SONG.song) + '/')];
  ...
  public function clearShaderFromCamera(cam:String){
	...
	private function generateSong(dataPath:String):Void
	{
		...
		#if MODS_ALLOWED
		if (FileSystem.exists(Paths.modsJson(songName + '/events')) || FileSystem.exists(Asset2File.getPath + file)) {
		#else
		if (OpenFlAssets.exists(file)) {
		#end
